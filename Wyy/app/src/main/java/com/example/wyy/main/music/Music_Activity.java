...
import com.example.wyy.classes.other.MusicService;
...
//    private static int flag=0,flag1=0;//用于判断checkbox的点击次数
//    MyServiceConn conn;
    MediaPlayer player;
    ...
    private void init(){
        ...
        //转换
        sListList=(List<SList>) intent1.getSerializableExtra("love");
     ...
     /*   bindService(intent2,conn,BIND_AUTO_CREATE);//绑定服务*/
        button.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                Intent intent=new Intent(Music_Activity.this,WordActivity.class);
                intent.putExtra("ly",address1);
                startActivity(intent,ActivityOptions.makeSceneTransitionAnimation(Music_Activity.this).toBundle());
            }
        });
        button1.setOnClickListener(new View.OnClickListener() {
            ...
            @Override
            public void onClick(View v) {
                Intent intent=new Intent(Music_Activity.this, CommentActivity.class);
                intent.putExtra("ly",address1);
                startActivity(intent,ActivityOptions.makeSceneTransitionAnimation(Music_Activity.this).toBundle());
            }
        });
             ...
             /*   player.release();//释放占用的资源*/
                ...
                for (int i = 0; i < sListList.size(); i++) {
                    if(sListList.get(i).getMusicId().equalsIgnoreCase(address1)){
                               int p=i;
//                        Log.d("SIZE1222222",String.valueOf(i));
                               p--;
                               address1=String.valueOf(sListList.get(p).getMusicId());
                        Glide.with(Music_Activity.this).load(sListList.get(p).getPicUrl()).apply(RequestOptions.bitmapTransform(new CircleCrop())).into(iv_music);
            ...
            public void onClick(View v) {
              /*  player.release();//释放占用的资源*/
                ...
                for (int i = 0; i < sListList.size(); i++) {
                    if(sListList.get(i).getMusicId().equalsIgnoreCase(address1)){
                        int p1=i;
//                        Log.d("SIZE123",String.valueOf(i));
                        p1++;
//                        Log.d("SIZE123333333333",String.valueOf(p1));
                        address1=String.valueOf(sListList.get(p1).getMusicId());
                        Glide.with(Music_Activity.this).load(sListList.get(p1).getPicUrl()).apply(RequestOptions.bitmapTransform(new CircleCrop())).into(iv_music);
